...
// V8 is not thread-safe; only allow one compile/run at a time via this mutex.
...
void v8_WasmMemoryObject_free(const Local<Context> &context, wasm32_ptr_t ptr) {
    ...
    // Isolate *isolate = context->GetIsolate();
    // Local<Object> memory_value = context->GetEmbedderData(kWasmMemoryObject).As<Object>();  // really a WasmMemoryObject
    // Local<ArrayBuffer> wasm_memory = Local<ArrayBuffer>::Cast(memory_value->Get(String::NewFromUtf8(isolate, "buffer")));
    // int32_t heap_base = context->GetEmbedderData(kHeapBase)->Int32Value(context).ToChecked();
...
struct JSVMModuleContents {
    ...
    v8::Persistent<v8::Function> v8_halide_malloc;
    v8::Persistent<v8::Function> v8_halide_free;
#endif
...
JSVMModuleContents::JSVMModuleContents(
...
#ifdef WITH_JSVM_V8
    std::lock_guard<std::mutex> lock_guard(v8_lock);
    ...
    if (!maybe_compiled.ToLocal(&compiled)) {
        // Some versions of V8 don't propagate the exception properly,
...
//print_object_properties(isolate, exports);
    ...
    context->SetEmbedderData(kNextAvailable, context->GetEmbedderData(kHeapBase));
...
JSVMModuleContents::~JSVMModuleContents() {
    ...
    if (isolate != nullptr) {
        // Not sure if this is required...
        {
            ...
            v8_context.Reset();
            v8_function.Reset();
            v8_halide_malloc.Reset();
            v8_halide_free.Reset();
        }
