...
D8 = d8 --experimental-wasm-se  --experimental-wasm-bulk-memory
...
$(BIN_DIR)/$(TARGET)/generator_aotwasm_%.js: $(ROOT_DIR)/test/generator/%_aottest.cpp $(FILTERS_DIR)/%.a $(FILTERS_DIR)/%.h $(RUNTIME_EXPORTED_INCLUDES) $(BIN_DIR)/$(TARGET)/runtime.a
	...
	@# TODO: use EMCC_WASM_BACKEND=1 ?
	$(EMCC) $(GEN_AOT_CXX_FLAGS_WASM) -s SIMD=0 -s EXIT_RUNTIME=1 -s ENVIRONMENT=shell --source-map-base . $(filter %.cpp %.o %.a,$^) $(GEN_AOT_INCLUDES) $(GEN_AOT_LD_FLAGS_WASM) -o $@
